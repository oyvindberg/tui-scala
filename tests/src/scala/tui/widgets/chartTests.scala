...
class chartTests extends TuiTest {
  ...
  test("it_should_hide_the_legend") {
    ...
    cases.foreach { c =>
      ...
      val chart = Chart(
        datasets = datasets,
        x_axis = Axis(title = Some(Spans.from("X axis"))),
        y_axis = Axis(title = Some(Spans.from("Y axis"))),
  ...
  def axis_test_case(width: Int, height: Int, x_axis: Axis, y_axis: Axis, lines: Array[String]): Unit = {
    val backend = TestBackend(width, height);
    val terminal = Terminal.init(backend)
    terminal.draw { f =>
      val chart = Chart(datasets = Array.empty, x_axis = x_axis, y_axis = y_axis)
  ...
  test("widgets_chart_handles_y_axis_labels_alignments") {
    def test_case(y_alignment: Alignment, lines: Array[String]): Unit = {
      val x_axis = Axis(labels = Some(create_labels(Array("AAAAA", "B"))))

      val y_axis = Axis(labels = Some(create_labels(Array("C", "D"))), labels_alignment = y_alignment)
  ...
  test("widgets_chart_can_have_empty_datasets") {
    ...
    terminal.draw { f =>
      val datasets = Array(Dataset(data = Array.empty, graph_type = GraphType.Line))
      val chart = Chart(
        datasets = datasets,
