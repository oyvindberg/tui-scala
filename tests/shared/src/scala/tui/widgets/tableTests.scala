...
class tableTests extends TuiTest {
  ...
  test("widgets_table_can_have_rows_with_multi_lines") {
    def test_case(state: TableState, expected: Buffer): Unit = {
  ...
  test("widgets_table_can_have_elements_styled_individually") {
    ...
    terminal.draw { f =>
      val table = Table(
        block = Some(Block(borders = Borders.LEFT | Borders.RIGHT)),
        ...
        rows = Array(
          ...
          Row(
            cells = Array(
              Cell(content = Text.raw("Row21")),
              Cell(content = Text.raw("Row22"), style = Style(fg = Some(Color.Yellow))),
              Cell(
  ...
  test("widgets_table_columns_dont_panic") {
    def test_case(state: TableState, table: Table, width: Int): Unit = {
