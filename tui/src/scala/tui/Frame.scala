...
/** Represents a consistent terminal interface for rendering.
  ...
  *   Where should the cursor be after drawing this frame. If `None`, the cursor is hidden and its position is controlled by the backend. If `Some((x, /// y))`, the cursor is shown and placed at `(x,
  *   y)` after the call to `Terminal.draw()`.
  */
case class Frame(
    buffer: Buffer,
    size: Rect,
