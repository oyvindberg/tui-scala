...
/** Primitives for styled text.
  *
  * A terminal UI is at its root a lot of strings. In order to make it accessible and stylish, those strings may be associated to a set of styles. `tui` has three ways to represent them:
  ...
  *
  * Keep it mind that a lot of widgets will use those types to advertise what kind of string is supported for their properties. Moreover, `tui` provides convenient `From` implementations so that you
  * can start by using simple `String` or `&str` and then promote them to the previous primitives when you need additional styling capabilities.
  *
  ...
  *
  * A `Text`, like a `Span`, can be constructed using one of the many `From` implementations or via the `Text.unstyled` and `Text.styled` methods. Helpfully, `Text` also implements `Extend` which
  * enables the concatenation of several `Text` blocks.
  */
