...
object Main {
  ...
  def ui(f: Frame, app: App): Unit = {
    ...
    val help_message = Paragraph(text = text)
    ...
    val input = Paragraph(
      ...
      block = Some(Block(borders = Borders.ALL, title = Some(Spans.from("Input"))))
    )
    ...
    val items: Array[ListItem] =
      app.messages.zipWithIndex.map { case (m, i) => ListItem(content = Text.from(s"$i: $m")) }

    val messages =
      tui.widgets.list.List(
        items = items,
        block = Some(Block(borders = Borders.ALL, title = Some(Spans.from("Messages"))))
      )
