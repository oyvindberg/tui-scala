...
public final class Solver {
  ...
  public Either<AddConstraintError, Void> addConstraint(Constraint constraint) {
    ...
    if (subject.tpe() == SymbolType.Invalid && allDummies(row)) {
  ...
  public Either<SuggestValueError, Void> suggestValue(Variable variable, double value) {
    ...
    if (rows.containsKey(infoTagMarker)) {
      ...
      for (Map.Entry<Symbol, Row> e : rows.entrySet()) {
        ...
        if (coeff != 0.0 && row.add(diff) < 0.0 && symbol.tpe() != SymbolType.External) {
  ...
  public Symbol getVarSymbol(Variable v) {
    ...
    if (value == null) {
      Symbol s = new Symbol(idTick, SymbolType.External);
  ...
  public RowAndTag createRow(Constraint constraint) {
    ...
    switch (constraint.op) {
      ...
      case LessOrEqual: {
        ...
        Symbol slack = new Symbol(idTick, SymbolType.Slack);
        idTick += 1;
        row.insertSymbol(slack, coeff);
        if (constraint.strength.compareTo(Strength.REQUIRED) < 0) {
          Symbol error = new Symbol(idTick, SymbolType.Error);
          idTick += 1;
      ...
      case Equal: {
        if (constraint.strength.compareTo(Strength.REQUIRED) < 0) {
          Symbol errplus = new Symbol(idTick, SymbolType.Error);
          idTick += 1;
          Symbol errminus = new Symbol(idTick, SymbolType.Error);
          idTick += 1;
          ...
          Symbol dummy = new Symbol(idTick, SymbolType.Dummy);
          idTick += 1;
  ...
  public Either<InternalSolverError, Boolean> addWithArtificialVariable(Row row) {
    ...
    Symbol art = new Symbol(idTick, SymbolType.Slack);
    idTick += 1;
  ...
  public void substitute(Symbol symbol, Row row) {
    for (Map.Entry<Symbol, Row> e : rows.entrySet()) {
      ...
      if (otherSymbol.tpe() == SymbolType.External && constantChanged) {
      ...
      if (otherSymbol.tpe() != SymbolType.External && otherRow.constant < 0.0) {
  ...
  public Either<InternalSolverError, Void> optimise(Row objective) {
    while (true) {
      ...
      if (entering.tpe() == SymbolType.External && sr.row.constant != 0.0) {
  ...
  public Symbol getEnteringSymbol(Row objective) {
    for (Map.Entry<Symbol, Double> e : objective.cells.entrySet()) {
      ...
      if (symbol.tpe() != SymbolType.Dummy && value < 0.0) {
        return symbol;
      }
    }
    return Symbol.invalid;
  ...
  public Symbol anyPivotableSymbol(Row row) {
    for (Symbol symbol : row.cells.keySet()) {
      if (symbol.tpe() == SymbolType.Slack || symbol.tpe() == SymbolType.Error) {
        return symbol;
      }
    }
    return Symbol.invalid;
  ...
  public Optional<SymbolAndRow> getMarkerLeavingRow(Symbol marker) {
    ...
    for (Map.Entry<Symbol, Row> e : rows.entrySet()) {
      ...
      if (c != 0.0) {
        // no-op (matches Scala `case _ if c != 0.0 => ()` branch)
    ...
    if (s.tpe() == SymbolType.External && this.rows.get(s).constant != 0.0) {
  ...
  public boolean allDummies(Row row) {
    for (Symbol s : row.cells.keySet()) {
      if (s.tpe() != SymbolType.Dummy) return false;
    }
    return true;
  }
